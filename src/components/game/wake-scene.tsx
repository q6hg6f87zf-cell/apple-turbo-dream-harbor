import { kaneEdgeFade, stillSrcAt, KANE_REEL, WAKE_REEL, type StillCue } from "@/game/opening-reel";
import { cueKaneVoiceFromPicture } from "@/game/radio";
...
/** Kane chapter: one office reel. Voice arms at 4s from picture time. */
export function KaneIntro({ time }: { time: number }) {
  const fade = kaneEdgeFade(time);

  return (
    <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden bg-ink" data-wake-clip="kane" aria-hidden>
      <WakeScene
        clip={KANE_REEL}
        sound={false}
        loop={false}
        veil="kane"
        preload="auto"
        onTime={(t) => cueKaneVoiceFromPicture(t)}
        onFail={() => cueKaneVoiceFromPicture(99)}
      />
