...
import { ITEM_KIND_PRESENTATION, classLabel, itemEmoji } from "@/game/presentation";
...
import type { InventoryCategory, Item, ItemKind, Operative, Rarity } from "@/game/types";
import { cn } from "@/lib/cn";
import { Archive, Check, CircleHelp, PackageCheck, Search, Sparkles, Wrench, X, Zap } from "lucide-react";
...
function tyrone(item: Item, target: Operative | null) {
  ...
  if (item.kind === "consumable") return "Use is permanent. The server consumes the item first, then I apply its field effect to the resident. Double taps cannot drink the same syringe twice.";
...
export function ServerInventoryView() {
  ...
  const rows = useMemo(() => {
    ...
    return base.filter((row) => {
      if (category !== "all" && row.item.kind !== category) return false;
      if (!q) return true;
      return [row.item.name, row.item.effect, row.item.rarity, row.item.kind, row.owner, row.item.classHint ? classLabel(row.item.classHint) : ""].join(" ").toLowerCase().includes(q);
  ...
  const resolvedGearId = targetGearId && gear.some((item) => item.id === targetGearId) ? targetGearId : gear[0]?.id ?? "";
  ...
  return (
    <div className="space-y-4 pb-8">
      <div className="flex items-end justify-between gap-3">
        <div>
          <SectionLabel>Vault 13 · server-sealed loadout</SectionLabel>
          <h2 className="font-display text-2xl">Inventory</h2>
          <p className="mt-1 text-sm text-muted">Tap → preview → act. Every owned item below has a server serial.</p>
        </div>
        <button type="button" onClick={() => setHelp((v) => !v)} className="flex min-h-11 items-center gap-2 rounded-[var(--radius-sm)] border border-ember/40 bg-ember/10 px-3 font-display text-[10px] uppercase tracking-[0.13em] text-ember">
          <CircleHelp className="size-4" /> Tyrone
        </button>
      </div>

      {help ? <Panel className="border-ember/30 bg-ember/5"><div className="flex items-start gap-3"><img src="/art/tyrone.jpg" alt="" className="size-12 rounded-[var(--radius-sm)] object-cover" /><p className="flex-1 text-sm leading-relaxed text-moon">Owned means the server can prove the item exists. Catalogue means we only know the design exists. LocalStorage is a clipboard now, partner, not a printing press.</p><button type="button" onClick={() => setHelp(false)}><X className="size-4 text-muted" /></button></div></Panel> : null}

      <div className="grid grid-cols-2 gap-2">
        <button type="button" onClick={() => { setMode("owned"); setSelectedKey(null); }} className={cn("min-h-14 rounded-[var(--radius-md)] border px-3 text-left", mode === "owned" ? "border-ember/60 bg-ember/10" : "border-line bg-raised")}><div className="font-display text-sm">🎒 Owned</div><div className="text-xs text-muted">{owned.length} server items</div></button>
        <button type="button" onClick={() => { setMode("catalogue"); setSelectedKey(null); }} className={cn("min-h-14 rounded-[var(--radius-md)] border px-3 text-left", mode === "catalogue" ? "border-ember/60 bg-ember/10" : "border-line bg-raised")}><div className="font-display text-sm">📚 Catalogue</div><div className="text-xs text-muted">{catalogue.length} known designs</div></button>
      </div>

      <div className="flex gap-2 overflow-x-auto pb-1">
        {CATEGORIES.map((id) => {
          const meta = id === "all" ? { emoji: "🧰", plural: "All" } : ITEM_KIND_PRESENTATION[id as ItemKind];
          return <button key={id} type="button" onClick={() => setCategory(id)} className={cn("shrink-0 rounded-full border px-3 py-2 font-display text-[10px] uppercase tracking-[0.12em]", category === id ? "border-ember/70 bg-ember/15 text-ember" : "border-line bg-raised text-muted")}>{meta.emoji} {meta.plural}</button>;
        })}
      </div>

      <label className="flex min-h-11 items-center gap-2 rounded-[var(--radius-md)] border border-line bg-raised px-3"><Search className="size-4 text-muted" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search gear, rarity, effect…" className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-muted" /></label>

      <div className="space-y-2">
        {rows.map((row) => <button key={row.key} type="button" onClick={() => { setSelectedKey(row.key); setHelp(false); setTargetGearId(""); if (!targetId && residents[0]) setTargetId(residents[0].id); }} className="flex min-h-16 w-full items-center gap-3 rounded-[var(--radius-md)] bg-raised px-3 py-3 text-left shadow-[var(--shadow-border)] transition active:scale-[0.99]"><span className="text-2xl">{itemEmoji(row.item.kind)}</span><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className="truncate font-display text-sm">{row.item.name}</span><RarityMark rarity={row.item.rarity} /></div><p className="truncate text-xs text-muted">{row.owner} · {row.item.condition}{row.item.equipped ? " · Equipped" : ""}</p></div><span className="text-xs text-moon">{row.item.value.toLocaleString()}c</span></button>)}
        {!rows.length ? <Panel><p className="text-sm text-muted">Nothing matches that filter.</p></Panel> : null}
      </div>

      {selected ? (
        <div className="fixed inset-0 z-[80] flex items-end justify-center bg-ink/80 p-2 md:items-center md:p-4" onClick={() => !pending && setSelectedKey(null)}>
          <div className="ms-pop max-h-[92dvh] w-full max-w-lg overflow-y-auto rounded-[var(--radius-xl)] bg-surface p-4 shadow-2xl ms-scroll" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start gap-3"><span className="text-3xl">{itemEmoji(selected.item.kind)}</span><div className="min-w-0 flex-1"><SectionLabel>{selected.owner}</SectionLabel><h3 className="font-display text-xl">{selected.item.name}</h3><div className="mt-1 flex flex-wrap gap-2 text-xs text-muted"><RarityMark rarity={selected.item.rarity} /><span>{selected.item.kind}</span><span>{selected.item.condition}</span>{selected.item.classHint ? <span>{classLabel(selected.item.classHint)}</span> : null}</div></div><button type="button" disabled={pending} onClick={() => setSelectedKey(null)} className="size-10 text-muted"><X className="size-5" /></button></div>

            <p className="mt-4 text-sm leading-relaxed text-paper">{selected.item.effect}</p><p className="mt-2 text-xs leading-relaxed text-muted">{selected.item.lore}</p>
            ...
            {selected.source !== "catalogue" ? <div className="mt-4 space-y-2">
              ...
              {selected.item.kind === "consumable" && target ? <Button variant="ember" className="w-full" disabled={pending} onClick={() => void act(() => consumeServerItem(selected.item.id, target.id), `${selected.item.name} used on ${target.name}.`, () => applyConsumableFieldEffect(selected.item, target.id))}><Zap className="size-4" /> Use on {target.name}</Button> : null}
            ...
            </div> : <p className="mt-4 text-center text-xs text-muted">Catalogue record only. Find the real item in the Hollow Realm.</p>}
          </div>
        </div>
      ) : null}
    </div>
