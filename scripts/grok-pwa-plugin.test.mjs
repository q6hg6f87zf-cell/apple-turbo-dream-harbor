...
test("emits og:image for a public host and prefers a custom card", () => {
  const empty = mkdtempSync(join(tmpdir(), "pwa-og-"));
...
test("placeholder og:image appends site.color when it is 6-digit hex", () => {
  const empty = mkdtempSync(join(tmpdir(), "pwa-color-"));
...
test("names the install page from host slug", () => {
  ...
  assert.equal(appNameFromHost("thehollowrealm.com"), "The Hollow Realm");
  assert.equal(appNameFromHost("www.thehollowrealm.com"), "The Hollow Realm");
});
...
test("renders Hollow Realm identity on the live host", () => {
  const manifest = JSON.parse(renderWebManifest("thehollowrealm.com"));
  assert.equal(manifest.name, "The Hollow Realm");
  assert.equal(manifest.short_name, "Hollow Realm");
  assert.equal(manifest.icons[0].src, "/icon-180.png");
  assert.ok(manifest.icons.some((icon) => icon.src === "/icon-512.png"));
});

test("renders Hollow Realm identity from the baked site title", () => {
  const manifest = JSON.parse(
    renderWebManifest("localhost:8080", { title: "The Hollow Realm" }),
  );
  assert.equal(manifest.name, "The Hollow Realm");
  assert.ok(manifest.icons.some((icon) => icon.src === "/icon-180.png"));
});
